...
def is_returning_msg_sender(function: Function) -> bool:
        ...
        for node in function.nodes:
            for ir in node.irs:     # traverse through each node' intermediate representation (IR)
                ...
                if (
                    ...
                    # if a=msg.sender, b=a, c=b 
                    # assignment maps stores `c:msg.sender` and  not `c:b`
        ...
        for function in contract.functions : 
            ...
            for call in function.all_internal_calls():
                try: 
                    ...
                    # we dont need solidity calls like revert, require ( dont have canonical_names)
                    continue
...
def get_function_trace_with_args_passed(sl: Slither):
    """
    Returns a dict of canonical-function-name -> (Function Trace list along with args passed).
    Trace list consists all functions that can be triggered from the context function.
    funcA -> [ (funcB,(x,y)), ... ]
    """
    trace = {}
