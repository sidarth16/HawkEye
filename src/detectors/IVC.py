...
def filter_function_without_proper_access_control(sl: Slither, result:dict):
    ...
    """
        Helper function which checks for proper access control 
    """
    ...
    # Run Checks on flagged_funcs from the result
...
def check_input_validation(function:Function, farg_vars=None):
    ...
    if len(ext_calls)>0:
        ...
        # get all ext calls and target contract variable
        ext_contract_vars = []
            ...
            for var in vars_to_check:
                ...
                for node in condt_nodes :
                    ...
                    if var.name in node : 
                        ...
                        # looks for calls inside conditions(if,require) that check name. symbol (if tokens)
                        # check for var.mint(), var.name(), , and ignore REGISTRY.legit(<var>)
                        if '.' in node and 'msg.'not in node:   # ignore msg.sender checks here
                            node = str(node)
                            ...
                            f_ind = node.find('.')
                            if f_ind:
                                # TODO: currently handles external token calls, need to extend to other types
            ...
            for function in contract.functions :
                ...
                if vars_to_check:
                    ...
                    for func, args_passed in ftrace[function.canonical_name]:
                        ...
                        # upd_vars_to_check handles this by replacing swapData -> _swap

                        upd_vars_to_check = []
                        tainted_vars_passed_as_arg = [] 
                        args_passed_var_name = [i.name for i in args_passed] #args passed by context func
                        for var in vars_to_check:
                            if var.name in args_passed_var_name:
                                ind = args_passed_var_name.index(var.name)
                                tainted_vars_passed_as_arg.append(func.parameters[ind]) # updated with the param name
                                upd_vars_to_check.append(func.parameters[ind])
                            else:
                                upd_vars_to_check.append(var)

                        # if any vars from `vars_to_check` are passed to the internal func
                        if any([i.name in args_passed_var_name for i in vars_to_check]):
                            # call `check_input_validation` with upd_vars_to_check as fargs
